...
2. **Next best — delegate the audit to a subagent.** If the harness supports dispatching one (OpenCode: a `subagent`-mode agent invoked via the `task` tool), dispatch it with instructions to invoke the `knowledge-org` skill and audit the whole tree and report findings, and give it *no* narrative about the session. It reads what is actually on disk, not what you meant to write. Then act on its findings yourself.
...
**Invoke the `knowledge-org` skill.** That is how you read the rules — every judgment made later in this workflow is a rule from it applied to a specific file — do not rely on memory of the rules.

Do **not** go looking for a bare `KNOWLEDGE_ORG.md` file on disk. This template ships the org rules only as the `knowledge-org` skill (`.opencode/skills/knowledge-org/SKILL.md`, or the global copy under `~/.config/opencode/skills/`) — there is no separate top-level file to find, and guessing at one wastes tool calls on a path that doesn't exist.

`AGENTS.md`'s rules are already in context — OpenCode loads it natively at session start.
