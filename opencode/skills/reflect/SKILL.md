...
2. For each changed file, ask:
   ...
   - **Status budget** — did a `status.md` "recent changes" entry grow past **300 characters**, or fail to end with a link? That is the most-violated rule in the whole system and it is violated *from inside this session*, by you, for reasons that felt good at the time. Count the characters (the whole entry, newlines collapsed to spaces) — don't eyeball line counts, since wrapping makes those mean nothing. If an automated status-budget check is active for this project it already flagged these at write time; this step is the backstop for when it isn't. Root cause, evidence, next steps, and security narrative all have homes that are not `status.md` (see the `knowledge-org` skill's "Recent-changes entries are pointers, not records"). Also check ordering: newest first, strictly.
   ...
   - **Open entries** — did an Open/backlog entry turn into an ordered or cross-domain plan sitting inline instead of being extracted to `phases.md`/`<domain>/plan.md` (see "Open entries are pointers, not plans")? And if this session *created* a plan file, is it at the right scope? A plan whose phases or driving decisions touch more than one domain goes in top-level `phases.md`, not `<domain>/plan.md` — the feature it delivers "belonging" to one domain is not the test (the `knowledge-org` skill).
...
2. **Find the canonical home before writing.** Quick grep the tree (or at least the relevant domain) for the fact — if it already lives in `status.md` or a dated audit, update or link there instead of restating it in a new place. The `knowledge-org` skill's no-duplication rule applies at write time, not just at curate time.
   ...
   **Then make it reachable, not just filed — for cross-cutting facts.** The right *home* is only half of placement. A cross-cutting constraint or preference (a budget rule, a language/style preference, a security limit) that will matter inside future tasks in *other* domains is buried if nothing links it from where it applies. So, after writing it to its canonical home, do both of these that fit:
   ...
   Keep it economical: a *pointer* in the guaranteed-read file, the full text in its home — do not paste the constraint's body into the mandatory files (that dilutes them). This is the `knowledge-org` skill's "Place for retrieval, not just for kind," applied at capture: the pointer is one line now and expensive to reconstruct a session later. User preferences and standing constraints are the facts this most often catches — they get stated once, mid-conversation, far from where they will next be needed.
