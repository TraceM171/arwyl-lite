...
**A moving conclusion is not yet a confirmed fact.** Before writing a candidate into `status.md`, `phases.md`, or a domain `_basic.md`, check: did I already write a different answer there this session? If so, it stays in the audit/decision file until checked against what's on record — full rule in `KNOWLEDGE_ORG.md`'s "A moving conclusion is not yet a status fact".
