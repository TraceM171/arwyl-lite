...
2. For each changed file, ask:
   ...
   - **Open entries** — did an Open/backlog entry turn into an ordered or cross-domain plan sitting inline instead of being extracted to `phases.md`/`<domain>/plan.md` (see "Open entries are pointers, not plans")? And if this session *created* a plan file, is it at the right scope? A plan whose phases or driving decisions touch more than one domain goes in top-level `phases.md`, not `<domain>/plan.md` — the feature it delivers "belonging" to one domain is not the test (`KNOWLEDGE_ORG.md`).
